...
/** Aligné sur Code.gs bfBoolsFromEmphasisStr_. */
function boolsFromEmphasisStr(key) {
...
export function parseFormOptions(inputs) {
  ...
  const emphasisRaw = readSelection(inputs, BF_FIELDS.TEXT_EMPHASIS);
  const biuForm = boolsFromEmphasisStr(emphasisRaw);
  let bold = false;
  let italic = false;
  let underline = false;
  if (biuForm) {
    ...
    bold = boldStr === '1';
    const italicStr = readSelection(inputs, BF_FIELDS.ITALIC) || '0';
    italic = italicStr === '1';
    const underlineStr = readSelection(inputs, BF_FIELDS.UNDERLINE) || '0';
