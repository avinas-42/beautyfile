...
describe('parseFormOptions', () => {
  ...
  it('parses selections and strings', () => {
    const inputs = {
      ...
      [BF_FIELDS.TEXT_EMPHASIS]: { selectionInputs: { value: ['bold_italic'] } },
  ...
  it('legacy separate bold/italic/underline fields when TEXT_EMPHASIS absent', () => {
    const inputs = {
      [BF_FIELDS.BOLD]: { selectionInputs: { value: ['1'] } },
      [BF_FIELDS.ITALIC]: { selectionInputs: { value: ['0'] } },
      [BF_FIELDS.UNDERLINE]: { selectionInputs: { value: ['1'] } },
    };
    const o = parseFormOptions(inputs);
    expect(o.bold).toBe(true);
    expect(o.italic).toBe(false);
    expect(o.underline).toBe(true);
  });
